...
# Modify the ensure_engine_loaded function
async def ensure_engine_loaded():
    global engine, engine_args, served_model_names, auto_load_enabled
    if engine is None and auto_load_enabled:
        try:
            ...
            served_model_names = [default_config["model"]]
            ...
            openai_serving_chat = OpenAIServingChat(
                ...
                model_config,
                served_model_names,
                "assistant",  # Default response role
                None,  # lora_modules
                None,
            )  # chat_template
            openai_serving_completion = OpenAIServingCompletion(
                engine, model_config, served_model_names, None
            )
            openai_serving_embedding = OpenAIServingEmbedding(
                engine, model_config, served_model_names
            )

            logger.info("Engine automatically loaded with default configuration.")
        except Exception as e:
            raise HTTPException(
                status_code=500, detail=f"Failed to automatically load engine: {str(e)}"
            )
    elif engine is None and not auto_load_enabled:
        raise HTTPException(
            status_code=400, detail="Engine is not loaded and auto-loading is disabled"
        )
...
async def toggle_autoload(enable: bool):
    global auto_load_enabled
    auto_load_enabled = enable
    return {"message": f"Auto-loading {'enabled' if enable else 'disabled'}"}
...
async def load_engine(config: Dict):
    global engine, engine_args, served_model_names

    if engine is not None:
        raise HTTPException(status_code=400, detail="Engine is already loaded")
    ...
    try:
        ...
        model_config = await engine.get_model_config()
        served_model_names = [config.get("model")]
        ...
        openai_serving_chat = OpenAIServingChat(
            ...
            model_config,
            served_model_names,
            engine_args.response_role,
            engine_args.lora_modules,
            engine_args.chat_template,
        )
        openai_serving_completion = OpenAIServingCompletion(
            engine, model_config, served_model_names, engine_args.lora_modules
        )
        openai_serving_embedding = OpenAIServingEmbedding(
            engine, model_config, served_model_names
        )

        return {"message": "Engine loaded successfully"}
    except Exception as e:
...
async def unload_engine():
    ...
    if engine is None:
        raise HTTPException(status_code=400, detail="No engine is currently loaded")
    ...
    try:
        ...
        return {"message": "Engine unloaded successfully"}
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to unload engine: {str(e)}"
        )
